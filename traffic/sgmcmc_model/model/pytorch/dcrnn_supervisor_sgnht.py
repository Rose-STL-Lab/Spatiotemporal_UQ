...
from model.pytorch.sgnht_gamma import Sampler
from model.pytorch.dcrnn_model import DCRNNModel
from model.pytorch.loss import masked_mae_loss, masked_rmse_loss

# Set visible device for cluster
# Please use os.environ["CUDA_VISIBLE_DEVICES"] = "" if we wish to use cpu
...
class DCRNNSupervisor:
    def __init__(self, adj_mx, **kwargs):
        ...
        # Parameter for SG-MCMC
        self.lr_anneal = 0.984
        self.anneal = 1.02
        self.sn = 100
    ...
    def load_model(self):
        ...
        dict = self.dist_of_param(self.dcrnn_model.parameters())
        count_list = []
        for key in sorted(dict):
            count_list.append(dict[key])
        print(sorted(dict))
        print(count_list)
        self._logger.info("Loaded model at {}".format(self._epoch_num))
        
    def setLoadEpoch(self, loadEpoch):
        loadEpoch = loadEpoch[3:]
        loadEpoch = loadEpoch[0:-4]
        print("Load epoch: ")
        print(loadEpoch)
        self._epoch_num = int(loadEpoch)
        self.load_model()

    def dist_of_param(self, params):
        val_dict = {}
        for param in params:
            for i in param.flatten():
                i = round(i.item(), 1)
    ...
    def _train(self, base_lr,
        ...
        # Tune down learning rate a little bit
        base_lr *= 0.005
        ...
        for epoch_num in range(self._epoch_num, epochs):
            ...
            for _, (x, y) in enumerate(train_iterator):
                ...
                if batches_seen == 0: 
                    sampler = Sampler(self.dcrnn_model, self._compute_loss, momentum=0.9, lr=base_lr, wdecay=0.0, total=23872, A = 10.0)

                loss = sampler.step(x, y, batches_seen)

                self._logger.debug(loss / 23872.0)
            ...
            if (epoch_num % log_every) == log_every - 1:
                message = 'Epoch [{}/{}] ({}) train_mae: {:.4f}, val_mae: {:.4f}, lr: {:.6f}, ' \
                          '{:.1f}s'.format(epoch_num, epochs, batches_seen,
                                           np.mean(losses), val_loss, sampler.eta,
                                           (end_time - start_time))
            ...
            if (epoch_num % test_every_n_epochs) == test_every_n_epochs - 1:
                ...
                message = 'Epoch [{}/{}] ({}) train_mae: {:.4f}, test_mae: {:.4f},  lr: {:.6f}, ' \
                          '{:.1f}s'.format(epoch_num, epochs, batches_seen,
                                           np.mean(losses), test_loss, sampler.eta,
                                           (end_time - start_time))
                self._logger.info(message)

            # if val_loss < min_val_loss or epoch_num >= 40:
