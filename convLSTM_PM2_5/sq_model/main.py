    ...
    for k in range (10,15):
        ...
        for i in range (50): 
            ...
            if (i+1)%10 == 0:
                ...
                np.save(os.path.join(path,'sq_epo{}.npy'.format(i)), outputs)
        ...
        for i in range (70): 
            ...
            if dev_loss < min_val_loss:
                ...
                if wait == patience:
                    outputs = ev.prediction(model,test_input_seqs)
                    np.save(os.path.join(path,'sq_epo{}.npy'.format(i+50)), outputs)
        ...
        torch.save(model.state_dict(), 'models/seed{}_epo{}.md'.format(k,i+50))
        losses = np.array(losses)
        dev_losses = np.array(dev_losses)
        test_losses = np.array(test_losses)
        np.save('../loss_sq/seed{}.npy'.format(k), np.stack([losses,dev_losses,test_losses]))
