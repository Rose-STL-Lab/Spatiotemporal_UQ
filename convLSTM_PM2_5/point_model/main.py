...
def main():
    ...
    for k in range (10):
        ...
        np.save(os.path.join(path,'point_epo{}.npy'.format(i)), outputs)
        ...
        for i in range (50): 
            ...
            if dev_loss < min_val_loss:
                ...
                if wait == patience:
                    outputs = ev.prediction(model,test_input_seqs)
                    np.save(os.path.join(path,'point_epo{}.npy'.format(i+50)), outputs)
        ...
        np.save('../loss_point/seed{}.npy'.format(k), np.stack([losses,dev_losses,test_losses]))
